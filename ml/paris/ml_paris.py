...
async def coletar_precos_ml_paris(hora_global, array_datas,data_atual):
    # Configurações do WebDriver Selenium
    driver = get_webdriver()
    dados = []
    wait = WebDriverWait(driver, 5)
    logging.info("Iniciando a coleta de preços ML Paris")
    try:
        for days in array_datas:
            future_date = get_future_date(days)
            url = f"https://www.vamonessa.com.br/ingressos/DISNEYLAND%20PARIS/354?destination=PARIS&destinationCode=41486&destinationState=Paris&destinationStateCode=70547&date={future_date}&provider=0"
            driver.get(url)
            ...
            xpath_pairs = [
                ...
                '3 Dias 2 Parques - Disney Paris'),
                ('//*[@id="root"]/div[2]/div[1]/div[3]/div[4]/div[1]/div[5]/div[2]/div[1]/button',
        ...
        nome_arquivo = f'paris_ml_{data_atual}.json'
        
        salvar_dados(df, nome_arquivo, 'paris/ml', hora_global)
        logging.info("Coleta de preços ML Paris concluída")
        #atualizar_calibragem(85)
        
if __name__ == '__main__':
    array_datas = [5,10,20,47,65,126]
    hora_global = datetime.now(pytz.timezone('America/Sao_Paulo'))
    asyncio.run(coletar_precos_ml_paris(hora_global,array_datas,'2024-03-25')) 