...
async def coletar_precos_voupra_nasa(hour,array_datas,data_atual):
    ...
    mapeamento_nomes = {
        184358: "NASA Kennedy Space Center",
        374601: "NASA Kennedy Space Center com Transporte",
        }
    
    for data in datas:
        url = f"https://www.voupra.com/estados-unidos/orlando/nasa-kennedy-space-center?Id=49340&Busca=true&DataTemporada={data}&dump=true"
        driver.get(url)
        time.sleep(2)
    ...
    nome_arquivo = f'nasa_voupra_{data_atual}.json'
    salvar_dados(df, nome_arquivo, 'outros/voupra', hour)
    
    #atualizar_calibragem(20)
    logging.info("Coleta de preços Voupra Nasa finalizada.")
    return